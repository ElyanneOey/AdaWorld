    ...
    with torch.no_grad():
        for batch_z, batch_actions, batch_games in loader:
            ...
            if batch_actions.ndim == 1:
                unique_actions = batch_actions.unique(sorted=True)
            else:
                unique_actions = batch_actions.unique(dim=0)
            ...
            for action_idx in unique_actions:
                if batch_actions.ndim == 1:
                    ...
                    action_mask = (batch_actions == action_idx).all(dim=1)
                    action_key = tuple(action_idx.cpu().numpy())
